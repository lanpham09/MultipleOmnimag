...
%% Orientation: Constantly pointing up

XDlist = zeros(length(Xlist),1);
YDlist = zeros(length(Ylist),1);
ZDlist = ones(length(Zlist),1);

%% Orientation: always tangent

% get gradients (slopes)
dx = gradient(Xlist);
dy = gradient(Ylist);
dz = gradient(Zlist);
...
for i=1:length(dx)
    ...
    if (vnorm > 0)
        ...
        % use former slope (probably zero)
        XDlist(i) = XDlist(i-1);
        YDlist(i) = YDlist(i-1);
        ZDlist(i) = ZDlist(i-1);
    end
...
% XDlist = zeros(length(Xlist),1);
% YDlist = zeros(length(Ylist),1);
% ZDlist = zeros(length(Zlist),1);
% dx = zeros(length(Xlist),1);
% dy = zeros(length(Ylist),1);
% dz = zeros(length(Zlist),1);
% vectornorm = zeros(length(Xlist),1);
% 
% XDlist(1) = 0;
% YDlist(1) = 0;
% ZDlist(1) = 0;
...
%     % get slope
%     dx(i) = Xlist(i) - Xlist(i-1);
%     dy(i) = Ylist(i) - Ylist(i-1);
%     dz(i) = Zlist(i) - Zlist(i-1);
%     
%     % normalize
%     vectornorm(i) = norm([dx(i) dy(i) dz(i)]);
%     
%     % update direction vector
%     if (vectornorm(i) > 0)
%         % use new slope
%         XDlist(i) = dx(i) / vectornorm(i);
%         YDlist(i) = dy(i) / vectornorm(i);
%         ZDlist(i) = dz(i) / vectornorm(i);
%     else
%         % if no change from last point, use old slope
%         XDlist(i) = XDlist(i-1);
%         YDlist(i) = YDlist(i-1);
%         ZDlist(i) = ZDlist(i-1);
%     end
...
fid = fopen('trajectory_tanj.csv','w');
for i = 1:length(Xlist) 
    fprintf(fid,'%6.6f, %6.6f, %6.6f, %6.6f, %6.6f, %6.6f\n', Xlist(i), Ylist(i), Zlist(i), XDlist(i), YDlist(i), ZDlist(i));
end
